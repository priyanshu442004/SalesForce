...
def resolve_mapping_columns(logic_df: pd.DataFrame) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"Mapping Logic file is missing required columns: {', '.join(missing)}")
...
def transform_source_data(source_path: str, logic_path: str, output_path: str) -> dict[str, Any]:
